...
export default function MapPage({ params }: any) {
  ...
  React.useEffect(() => {
    setBranchState((prev) => ({
      ...
      campaign: 'bill_map',
      deepLinkPath: '',
      desktopUrl: `${process.env.DESKTOP_URL}/bills/${params.id}/map?bill_type=${bill_type}`,
      feature: 'bill-map',
    }));
  ...
  return (
    <div className='relative md:pl-4 mt-10 md:mt-0 md:h-[100%] overflow-hidden'>
      ...
      <div
        className={cn(
          'h-[calc(100vh_-_70px)] overflow-auto bg-white shadow-lg absolute top-0 transition ease-in-out duration-300 hidden',
          {
