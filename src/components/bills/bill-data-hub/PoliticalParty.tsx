...
import { ChartSkeleton } from '@/app/(private)/bills/[id]/data-hub/page';
...
const PoliticalParty = (props: Props) => {
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: ['get-political-party-votes'],
    queryFn: () => getVoteAnalytics(params, { billType }),
    refetchOnWindowFocus: false,
  });

  const chartData = ['democrat', 'republican', 'independent', 'other'].map(
    (category) => ({
